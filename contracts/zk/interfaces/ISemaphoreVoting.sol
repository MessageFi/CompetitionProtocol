...
interface ISemaphoreVoting {
    ...
    function createPoll(address coordinator, uint256 merkleTreeDepth) external returns(uint256);

    /// @dev Return the state of the poll.
    /// @param pollId: Id of the poll.
    function pollState(uint256 pollId) external returns(PollState);
